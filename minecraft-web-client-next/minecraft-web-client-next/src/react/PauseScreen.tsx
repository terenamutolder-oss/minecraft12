...
export default () => {
  ...
  const localAppUrl = `${window.location.origin}${window.location.pathname}`
  const pauseInviteLink = singleplayer && wanOpened ? getJoinLink() : undefined
    ...
    <div className={styles.pause_container}>
      ...
      {singleplayer ? (
        <>
          ...
          >
            <div>Local link: {localAppUrl}</div>
            {pauseInviteLink
              ? <div style={{ marginTop: 4 }}>Invite link: {pauseInviteLink}</div>
              : null}
          </div>
        </>
      ) : null}
